...
def update_team_ratings_for_yesterday(target_date=None):
    ...
    for team_id in teams:
        # Compute season-to-date ratings through target_date (idempotent, safe for backfills).
        cur.execute(
            """
            SELECT 
        ...
        games_played = game_totals[0] or 0
        points_for = game_totals[1] or 0
        points_against = game_totals[2] or 0
        wins = game_totals[3] or 0
        ...
        fga = team_stats[0] or 0 if team_stats else 0
        oreb = team_stats[1] or 0 if team_stats else 0
        tov = team_stats[2] or 0 if team_stats else 0
        fta = team_stats[3] or 0 if team_stats else 0

        total_possessions = fga - oreb + tov + 0.44 * fta
        if total_possessions == 0:
            continue

        offensive_rating = round((points_for / total_possessions) * 100, 1)
        defensive_rating = round((points_against / total_possessions) * 100, 1)
